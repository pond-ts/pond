 ...
 * `dashed` / `step` / `fade`) on the gap-aware layers (line + area; bands have no
 * gap mode). Each story stacks the five modes top→bottom over one gapped series,
 * so a single snapshot captures all five renderings of the same coast side by
 * side. Snapshots the whole story root (`#storybook-root`) — multiple rows + DOM
