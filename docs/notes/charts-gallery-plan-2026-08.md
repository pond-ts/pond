...
- **C4 confirmed how the cursor behaves on an ordinal axis**: `crosshair`
  degrades to a vertical line plus the hovered sector's name, which is what
  the 16-sector rose needs (the axis only prints 8 labels). `line` shows
  neither. `[PND-CHFRIC]` 16 and 18.
