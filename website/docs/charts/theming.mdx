...
| `brush?`                                                     | no          | `{ fill, edge? }` — the drag band (see [below](#bar-interaction-states))                                                       |
...
## Bar interaction states {#bar-interaction-states}

A bar is the one mark that carries an **interaction state** — it can be at
rest, under the pointer, selected, or outside somebody else's selection. Four
states cannot be read as four shades of one colour, so `defaultTheme`'s `bar`
slot is a small palette in which **state is a hue difference**:
