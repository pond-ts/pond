...
difference — an _omitted_ `height` means **unmanaged**, the classic mode where
...
the [resizable panels recipe](../recipes/resizable-panels) for that shape,
...
a flex-_column_ child's height defaults to its content, so the unconstrained-
...
(`range`, `theme`, the cursor components, and the axis-kind props are covered under
...
| `children` | `ReactNode` | —       | `<YAxis>` (left) · `<Layers>` · `<YAxis>` (right), in order. A cursor component here overrides the container's for this row. |
