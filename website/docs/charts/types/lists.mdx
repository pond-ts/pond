...
| `hovered` / `onRowHover`             | `string \| string[] \| null` / `(row \| null)` | —           | Controlled hover-highlight + the outbound half — light rows from outside the list, or mirror the list's hover out.                                                        |
...
[`<ChartContainer hovered>`](../interaction/selection-and-hover.mdx) takes, so a
