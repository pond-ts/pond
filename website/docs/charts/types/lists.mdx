...
An `aggregate` produces one event per split; `listRowsFromTimeSeries` lifts
every value column (numeric **and** string) into `values` under its own name.
Distance-keyed splits (`byValue('km')`) go through `listRowsFromValueSeries`
identically.
...
const rows = listRowsFromTimeSeries(splits, { label: (i) => `${i + 1}` });

<BarList rows={rows} columns={[{ column: 'speed' }]} />;
```
