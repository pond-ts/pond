...
- **`<ChartContainer>`** — owns the shared x axis (time or value, inferred from
  the data), the view `range`, overall `width`, and `theme`. A cursor is a
  component mounted inside it (`<LineCursor />`, `<CrosshairCursor />`, …);
  mount none for no cursor.
...
[CHANGELOG](https://github.com/pond-ts/pond/blob/main/CHANGELOG.md) for
...
[PLAN.md](https://github.com/pond-ts/pond/blob/main/PLAN.md)'s
