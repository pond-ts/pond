...
One row per _entity_ rather than per bucket; `listRowsFromTimeSeries` /
`listRowsFromValueSeries` read a per-event series straight into rows, and
partition facts (`partitionBy` + `reduce`) spread straight into a row's
`values` — see [the acquisition recipes](./types/lists.mdx#from-your-timeseries).
