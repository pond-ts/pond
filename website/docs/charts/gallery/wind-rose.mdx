...
own axis label. That's what `<CrosshairCursor />` does on an **ordinal** axis:
...
| ----------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<RangeCursor onDragRelease>` | Drag anywhere on the plot to select a span      | You want free-form span selection and don't need a mark that persists — it's the [rainfall card's](./rainfall.mdx) gesture, and it needs the cursor `editAnnotations` takes away |
...
- [Cursors and readouts](../interaction/cursors-and-readouts.mdx) — what each cursor component draws
