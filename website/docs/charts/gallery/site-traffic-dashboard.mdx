...
<Layers>
  ...
  {/* …and, when a row is picked, its contribution within it. One conditional
      per layer, NOT a fragment around both: `<Layers>` injects each child's
      declaration index to set the z-stack, and a fragment takes no props, so
      both overlays would register at 0 and end up *behind* the second wash. */}
  {picked && (
