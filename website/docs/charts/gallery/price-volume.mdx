...
import GalleryVolumeList from '@site/src/examples/gallery-volume-list';

## The chart

<ChartExample name="gallery-price-volume" height={330} collapsed>
  <GalleryPriceVolume width={640} />
</ChartExample>

Move the cursor: **one** crosshair, spanning both rows, its value pill pinned to
the axis of whichever row the pointer is in. **Drag to pan, wheel to zoom** —
both rows follow together, because the x range lives on the container and the y
scales don't.

The strip above the chart is a second, off-chart readout, and it exists because
the in-chart one can't do this: the crosshair pill reads _one_ row at a time,
...
Which sessions those are is a question the volume _row_ can't answer — a bar at
a time position tells you when, not which. Ranked, the rows stop being buckets
on an axis and become **entities**, which is what the list family is for:

<GalleryVolumeList />

That's a [`<BarList>`](../types/lists.mdx), not a hand-built table: one bar
...
[`<Baseline>`](../annotations/region-baseline-marker.mdx) and draws in the
annotation register rather than a data hue.

Read the rows and the model shows through. The **top four are all losses**, and
three of them (Mar 23 / 25 / 26) are consecutive — that's volume answering a
move _and_ the decay afterwards, not a same-day spike. But it isn't a one-sided
rule: two of the eight are strong up sessions and one is flat, because what
volume responds to is the **size** of the move, not its sign.
...
<ChartContainer range={range} width={680} theme={theme}>
  ...
  <ChartRow height={170}>
    ...
    <Layers>
      <AreaChart series={bars} column="close" axis="price" baseline="floor" />
    </Layers>
  </ChartRow>
  <ChartRow height={74}>
...
Interaction is three props on the container plus a cursor component, and they
interlock:
...
`<CrosshairCursor />` gives you the in-chart readout: a shared vertical line
across both rows, and a value pill pinned to the y-axis of the row the pointer
is in. Note what that _isn't_ — it reads **one row at a time**. Both rows'
...
That's the strip above the chart. The two readouts are complementary rather
than alternatives: the pill is where your eye already is, the strip is the one
place both quantities appear together.
...
| Drop `baseline="floor"` on the area       | Fills from zero instead of the axis floor           | The row is a _change_ or a spread, where zero is meaningful             |
...
| `<RangeCursor>`                           | Drag selects a span, via `onDragRelease`            | Drag should mark a range rather than pan (it replaces the crosshair)    |
