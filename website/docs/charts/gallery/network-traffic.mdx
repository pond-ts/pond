...
| ------------------------------------------------ | ------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `<CrosshairCursor />` in `<ChartContainer>`      | Both halves read out at the hovered time                            | Always, on a mirrored chart — the pair of numbers is the point               |
...
| `baseline="floor"` on the `out` layer            | It fills to the axis floor instead of the zero line                 | Never here — zero is the default, and the negated `out` column hangs from it |
