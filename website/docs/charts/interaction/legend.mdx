...
## Headless — `useChartLegend()` \{#headless-usechartlegend\}
...
- [Selection & hover](./selection-and-hover.mdx) — the id-gated contract the
  legend's interactions ride on. Note that a legend click is a **programmatic**
  select, so its `onSelect` fires with `modifiers` **undefined** — that absence
  is how a handler tells a chip from a pointer gesture.
- [Sweeps & multi-select](./sweeps-and-multi-select.mdx) — the plural currency,
  if your legend drives a multi-selection.
