...
**2. The selector reports; the container holds the state.** `selected` and
`hovered` stay on `<ChartContainer>`, so controlled highlighting — a legend
chip, an external filter list, a linked table — keeps working with **no**
selector mounted at all. The break narrows to precisely the thing being made
deliberate: the plot gesture.
...
Mount the selector inside a `<ChartRow>` instead of the container to scope it to
**that row's** clicks; as a direct child of the container it hears every row.
...
- **A selector must be mounted.** `<ChartContainer onSelect>` / `onHover` still
  work for one more minor and warn; they are the pre-`<Selector>` shape.
