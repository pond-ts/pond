...
assignability _is_ the contract. The one exception is deliberate: a `boolean`
or array-kind column, which only a `byValue` projection can introduce, exports
fine but can't be ingested back by `fromColumns`, and the types say so at
