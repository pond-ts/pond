...
import CoreAlign from '@site/src/examples/core-align';
...
<ConceptViz
  ...
  caption={
    <>
      The grid splits time into <strong>steps</strong> — the regions between the
      orange lines. <code>sample</code> picks{' '}
      <strong>where in each step</strong> to read: its <code>begin</code>,{' '}
      <code>center</code>, or <code>end</code>. At that read-line,{' '}
      <code>hold</code> takes the previous sample (to its <strong>left</strong>
      ); <code>linear</code> interpolates the two samples{' '}
      <strong>around</strong> it. The teal dots are the real <code>align</code>{' '}
      output there; the faint lines are the construction. (See{' '}
