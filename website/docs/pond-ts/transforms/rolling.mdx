...
<ConceptViz
  ...
  caption={
    <>
      ...
      <code>window</code> to smooth more or less. Live rolling is{' '}
      <strong>trailing-only</strong>: with no future events to average, the
      window can only look back, so the anchor always sits at its leading edge.
      (See <a href="/docs/charts/">@pond-ts/charts</a> for how the chart is
      built.)
    </>
  }
>
