...
All four render as `Charts/BarChart/Histogram` stories in the `@pond-ts/charts`
...
// Five hosts is past the point where distinct hues help — a five-colour stack
// reads as a pie chart. Take a **sequential ramp** instead: one hue family
// stepping lighter, straight off the theme, so it flips with dark mode and
// nothing here is a literal.
const ramp = useSequentialRamp(HOSTS.length); // ['#0c444b', … '#bbe8df']
const hostColors = Object.fromEntries(HOSTS.map((h, i) => [h, ramp[i]]));
...
<ChartContainer range={[hourStart, hourEnd]} width={660}>
  ...
  <ChartRow height={260}>
    ...
    <Layers>
      <BarChart series={byHost} column="n" colors={hostColors} gap={2} />
    </Layers>
  </ChartRow>
</ChartContainer>;
```

:::note[Where the ramp comes from]
`useSequentialRamp` is this site's own helper
(`website/src/theme/useSiteChartTheme.ts`) reading `--pond-viz-seq-1…8` — the
docs theme's tonal ramp. There's no ramp _in the library_: `@pond-ts/charts`
ships the `ChartTheme` type and one renderable default, and a consumer owns
its palette. The transferable part is the rule, not the helper: **≤ 4 series
categorical, > 4 sequential**, and the colours come from the theme so a
dark-mode toggle moves them.
:::
...
Three bands with a **meaning** each — good, watch, bad — is the case a ramp is
wrong for and a stable vocabulary is right for. Name them in the theme once and
the `colors` prop disappears:

```ts
// in your ChartTheme — green / amber / red, read from your design tokens so
// they follow dark mode like everything else
bar: {
  default: { fill: v('--pond-viz-1'), highlight: v('--pond-viz-2') },
  safe: { fill: v('--pond-viz-up') },
  warn: { fill: v('--pond-viz-mark') },
  crit: { fill: v('--pond-viz-down') },
}
```
...
A segment whose group name matches a `bar` role picks that role up on its own —
no prop. Because `crit` is last in `RISK_BANDS`, red stacks on top: as usage
drifts up over the hour the stack visibly reddens from the top down.

### Colours: `colors` prop vs theme roles

The two examples above took the two paths, and which one is right follows from
whether the groups are a **vocabulary** or a **list**:

- **theme roles** (example 2) — name the groups as `bar` styles in your theme
  (`theme.bar.safe`, `theme.bar.warn`, …) and drop the `colors` prop. Best when
  the groups are a stable vocabulary the whole app shares, and each one means
  something.
- **`colors={{ group: hue }}`** (example 1) — the ad-hoc path, for groups you
  won't name in a theme because there's nothing to say about `web-2` that isn't
  also true of `web-3`. Feed it from the theme's ramp rather than from
  literals; the point of the prop is the _keying_, not the hardcoding.
...
up. Both channels are controllable via a mounted `<Selector>`'s
...
<ChartContainer>
  <LineCursor />
  <Selector onSelect={(hit) => hit && console.log(hit.label, hit.value)}>
    …
    <BarChart series={byHost} column="n" colors={hostColors} id="incidents" />
  </Selector>
</ChartContainer>
```

Hover and click work in both orientations (they hit-test by pixel). The in-chart
`<FlagCursor>` / `<CrosshairCursor>` value cursor — the one that scrubs along x — is drawn for
