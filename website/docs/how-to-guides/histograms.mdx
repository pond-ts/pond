...
- **Stacks are non-negative.** Segment values are treated as counts / durations;
  a negative value is dropped (diverging stacks are out of scope). For a single
  diverging series (up _and_ down off zero), use a plain `column` bar — see the
  `BarChart` `Diverging` story.
