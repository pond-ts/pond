...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `ChartContainer`            | `width`, `range?`, `theme?`, `cursor?`, `panZoom?`, `bounds?`, `showAxis?`, `calendar?`, `origin?`, `maxBandWidth?`/`bandAlign?`, `selected?` (mark \| set), `onSelect?` (deprecated → `<Selector>`), `onTrackerChanged?`, `onDrawStats?` | Root: shared x-scale, interactions, annotations                                                                                             | `packages/charts/src/ChartContainer.tsx`               |
...
| `Selector`                  | `onSelect?`, `onHover?`                                                                                                                                                                                                                   | Mount to enable click-select; state stays on the container                                                                                  | `packages/charts/src/selectors.tsx`                    |
| `MultiSelector`             | `sequence?`, `onSelect?`, `onHover?`                                                                                                                                                                                                      | Sweep-select superset of `Selector`: drag sweeps marks, release reports `(hits, modifiers, spans)` — plural, one per swept layer (RFC A5.2) | `packages/charts/src/selectors.tsx`                    |
