...
| Plan — request/response | `RunRequest`, `RunResult`, `Select` (`ColumnSelect` \| `ReduceSelect`), `ReductionName` (`last`, `extremes`, `percentileRank`, `shape`), `ErrorPolicy`, `Fact`, `OutputInfo`, `Skipped`                                   | `packages/process/src/plan/run.ts`             |
| Plan — host             | `createHost({ registry, units })` → `Host` (`add`, `has`, `datasets`, `graphFor`, `run`), `toWire`, `UnknownDatasetError`; types `Envelope` (`{ from, process, as }`), `DatasetInfo`, `WireResult`                        | `packages/process/src/plan/host.ts`            |
